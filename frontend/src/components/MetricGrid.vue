...
<template><section class="metric-section"><div class="metric-section-head"><strong>当前指标</strong><span>变化量：当前趋势时间范围内，首个采样点至今</span></div><div class="metric-grid"><div v-for="([key,label]) in metrics" :key="key" class="metric-card"><span>{{label}}</span><strong>{{format(latest?.[key])}}</strong><em :class="{negative:(growth?.[key]||0)<0}">{{(growth?.[key]||0)>0?'+':''}}{{format(growth?.[key]||0)}}</em></div></div></section></template>
